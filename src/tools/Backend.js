...
export default class Backend {
    ...
    static __call(method, params, httpMethod = "GET") {
        ...
        return new Promise( (resolve, reject) => {
            try {
                fetch(url, requestParams).then(resolve).catch(reject)
            } catch (e) {
                reject(e)
            }
        } )
    ...
    static request(method, params, httpMethod = "GET", retry = 20) {
        return new Promise((resolve, reject) => {
            try {
                Backend.__call(method, params, httpMethod)
                    ...
                    .catch(e => {
                        if (e && e.message === 'Failed to fetch' && retry > 0) {
                            setTimeout( () => {
                                Backend.request(method, params, httpMethod, retry-1)
                                    .then(resolve)
                                    .catch(reject)
